...
function addASMHandlers() {
    ...
    $("input[name='customerName'], input[name='customerId']").hover(function() {
            ...
            if( !(item === null || item === undefined) ) {
                $(this).after( '<div id="asmHover" class="'+ disabled +'"><span class="name">' + item.name + '</span><span class="email">' + item.email + '</span><span class="date">' + item.date +'</span><span class="card-id">' + item.card + '</span></div>' );
            }
        }, function () {
            removeAsmHover();
        }
    ...
    if ($("input[name='customerName']").length > 0) {
        $("input[name='customerName']").autocomplete({
            source: function( request, response ) {
                $.ajax({
                    url: TPS.utils.getInternationalUrl("/assisted-service/autocomplete"),
                    dataType: 'json',
                    data: {
            ...
            select: function( event, ui ) {
                ...
                $.ajax({
                    url: TPS.utils.getInternationalUrl("/assisted-service/get-carts"),
                    dataType: 'json',
                    data: { customerId: ui.item.email },
                    success: function( data ) {
                        carts = data;
            ...
            return $( "<li></li>" )
                ...
                .append( "<a><span class='name'>" + item.value + "</span><span class='email'>" + item.email +
                    "</span><span class='date'>" + item.date + "</span><span class='card-id'>" + item.card + "</span></a>")
                .appendTo( ul );
        };
    }
    ...
    if ($(".add_to_cart_form").length && $("#_asm input[name='cartId']").val() == "") {
        $( ".add_to_cart_form" ).submit(function( event ) {
            setTimeout(function () {
                var url = TPS.utils.getInternationalUrl("/assisted-service/add-to-cart");
...
function addCloseBtnHandler() {
    $("#_asm .closeBtn").click( function() {
        var url = TPS.utils.getInternationalUrl("/assisted-service/quit");
        $.post(url, function( data ) {
...
function resetSession() {
    var request = $.ajax({
        url : TPS.utils.getInternationalUrl("/assisted-service/resetSession"),
        type : "POST",
        data: {CSRFToken: TPS.CSRFToken}
    });
...
function finishASMagentSession() {
    $.ajax({
        url :TPS.utils.getInternationalUrl("/assisted-service/logoutasm"),
        type : "POST",
        success: function(data) {
            $("#_asm").replaceWith(data);
            addASMHandlers();
